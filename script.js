...
async function getMoreInfo(id) {
  const options = {
    ...
    headers: {
      ...
      Authorization:
        "Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIwMjIyYTM4ZGI1ZWY2M2MxODcyN2IwZjIzMDE5ZTNlNSIsInN1YiI6IjY0MmQ2OTVhMGQyZjUzMDBmNGY3NTA2OCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.QCEfjurYMel6lHJXrUy2rqwaRXysl7YEEU_9PW6cVdk",
    },
  };
...
async function watch(event) {
  ...
  try {
    ...
    window.open(`http://youtube.com/watch?v=${youtubeVideo.key}`, "blank");
  } catch (error) {
    console.log(error);
  }
}
...
function createMovieLayout({ id, title, stars, image, time, year }) {
  ...
  <div class="info">
    ...
    <div class="year">
      <img src="./assets/data.svg" alt="calendar" />
      <span>${year}</span>
    </div>
  </div>
...
function select3Videos(results) {
  ...
  let selectedVideos = new Set(); // Set é uma estrutura de dados que não aceita repetição
  while (selectedVideos.size < 3) {
    selectedVideos.add(results[random()].id); // add adiciona um elemento no set
  }
